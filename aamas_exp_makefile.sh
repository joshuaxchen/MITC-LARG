...
${SETPATH} ${RUN} ${PPO_RUNNER} --benchmark_name merge4_Sim --num_cpus ${CPU_NUM} --lr ${LR} --num_rollouts ${NUM_ROLLOUTS} 

echo "2nd exp: train agent with reward as the number of departing vehicles (outflow)"
${SETPATH} ${RUN} ${PPO_RUNNER} --benchmark_name merge4_Sim_Arrive --num_cpus ${CPU_NUM} --lr ${LR} --num_rollouts ${NUM_ROLLOUTS} 
