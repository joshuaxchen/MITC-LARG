...
class SingleLaneSimpleMerge:
    ...
    def get_inflow_according_to_cell_model(self, upstream_cell_capacity, current_veh, maximum_capacity, maximum_flow, shockwave_speed, freeflow_speed):
        # assume tha 
        return min(upstream_cell_capacity, maximum_flow, shockwave_speed*(maximum_capacity-current_veh)/freeflow_speed)
    ...
    def step(self, specified_main_inflow, specified_merge_inflow):
        ...
        for i in range(0, self.main_cell_num):
            ...
            if i==0: # the first cell has inflow maximum
                ...
                main_n_previous_cell_t=waiting_merge_veh+main_n_previous[i-1]
            else: 
                main_n_previous_cell_t=main_n_previous[i-1] # assume all vehicles will transit to the next
            main_n_current_cell_t=main_n_previous[i]
            #alpha=0
            #if main_n_previous_cell_t<=self.Q or i==0:
            #    alpha=1
        ...
        ***********************************
        ...
        if waiting_merge_veh>0: 
            ...
            if main_n_previous[self.before_merge_index]>=threshold:
            ...
            if main_n_previous[self.before_merge_index]>=threshold:
                if self.remaining_waiting>=1:
                    ...
                    main_inflows[self.before_merge_index+1]=main_inflows[self.before_merge_index+1]*(1-self.remaining_waiting)
                else:
                    main_inflows[self.before_merge_index+1]=0
        ...
        for i in reversed(range(0, self.main_cell_num)):
            ...
            if i==self.main_cell_num-1:# all vehicles will go out
                ...
                outflow=max(0, main_inflows[i+1]-waiting_merge_veh)
            else:
                outflow=main_inflows[i+1] # inflow from downstream
            #if i==self.before_merge_index:
            #    if waiting_merge_veh>0 or self.remaining_waiting>=1: # no waiting vehicle but been blocked by previous time steps
            #        outflow=0
            #    elif self.remaining_waiting>0:
            #        outflow=main_inflows[self.before_merge_index+1]
