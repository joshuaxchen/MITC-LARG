...
def plot_again_dist(mor_summary, human_summary):
    ...
    for eval_key, eval_value in mor_summary.items():
        ...
        if main_merge_avp_text not in mor_data:
            mor_data[main_merge_avp_text]=list()
        (mean,var)=extract_mean_var(eval_value, attr_name)
        mor_data[main_merge_avp_text].append((dist, mean, var))
        mor_category_list.append(main_merge_avp_text)

    human_category_list=list(set(human_category_list))
    human_category_list.sort() 
    mor_category_list=list(set(mor_category_list))
...
if __name__ == "__main__":
    ...
    mor_summary=retrive_evaluations(mor_dir)
