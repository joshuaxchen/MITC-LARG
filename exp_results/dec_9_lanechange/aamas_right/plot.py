...
for AVP in [10, 20, 30, 40]:
    ...
    plt.plot([1600, 1800, 1900, 2000], results, label=f"AVP={AVP}")
    results = []
    for r_inflow in [1600, 1800, 1900, 2000]:
        results.append(file_results[template_human.format(r_inflow, left_inflow, 200, AVP)])
    plt.plot([1600, 1800, 1900, 2000], results, label=f"HUMAN AVP={AVP}")
...
for r_inflow in [1600, 1800, 1900, 2000]:
    ...
    for AVP in [10, 20, 30, 40]:
        results.append(file_results[template.format(r_inflow, left_inflow, 200, AVP)])
    plt.plot([10, 20, 30, 40], results, label=f"Right Inflow={r_inflow}")
    results = []
    for AVP in [10, 20, 30, 40]:
        results.append(file_results[template_human.format(r_inflow, left_inflow, 200, AVP)])
    plt.plot([10, 20, 30, 40], results, label=f"HUMAN Right Inflow={r_inflow}")
