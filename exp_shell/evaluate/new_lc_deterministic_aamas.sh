...
do
	...
	do
		...
		do
			...
			if [[(AVP_LEFT==0) && (AVP_RIGHT==0)]]; then
				let NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT=HUMAN_INFLOW_RIGHT*10/100		
				let HUMAN_INFLOW_RIGHT=HUMAN_INFLOW_RIGHT-NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT
			fi
			echo ${RL_INFLOW_RIGHT} ${RL_INFLOW_LEFT} 
			echo ${HUMAN_INFLOW_RIGHT} ${HUMAN_INFLOW_LEFT} ${NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT}
