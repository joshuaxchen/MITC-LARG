...
EXP_FOLDER=$FLOW_DIR/exp_results/new_lc_deterministic_action_random_lane_change/
...
WORKING_DIR=$EXP_FOLDER/aamas_right_on_right
mkdir ${WORKING_DIR}
...
do
	...
	do
		...
		do
			...
			NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT=0
			if [[(AVP_RIGHT -eq 0)]]; then
				let NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT=RIGHT_MAIN_INFLOW*10/100		
				let HUMAN_INFLOW_RIGHT=HUMAN_INFLOW_RIGHT-NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT
				echo "avp right is 0"
			fi

			# add no lane changing vehicles at the left lane	
			NO_LANCHANGE_HUMAN_INFLOWS_ON_LEFT=0
			if [[(${AVP_LEFT} -eq 0)]]; then
				let NO_LANCHANGE_HUMAN_INFLOWS_ON_LEFT=LEFT_MAIN_INFLOW*10/100		
				let HUMAN_INFLOW_LEFT=HUMAN_INFLOW_LEFT-NO_LANCHANGE_HUMAN_INFLOWS_ON_LEFT
				echo "avp left is 0"
			fi
			echo ${HUMAN_INFLOW_LEFT} ${RL_INFLOW_LEFT} ${NO_LANCHANGE_HUMAN_INFLOWS_ON_LEFT}
			echo ${HUMAN_INFLOW_RIGHT} ${RL_INFLOW_RIGHT} ${NO_LANCHANGE_HUMAN_INFLOWS_ON_RIGHT}
