...
class MergePOEnvWindowSizeArrive(MergePOEnv):
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        for p in ADDITIONAL_ENV_PARAMS.keys():
            if p not in env_params.additional_params:
                raise KeyError(
                    'Environment parameter "{}" not supplied'.format(p))
        if "window_size" not in env_params:
            raise KeyError(
                    'Environment parameter "{}" not supplied'.format("window_size (:istance before the junction)"))
        ...
        self.window_size=env_params.additional_params["window_size"]
