...
def add_veh_and_inflows_to_edge(inflows, vehicle_params, edge, rl_inflows, rl_lane_change, human_inflows, human_lane_change):
    ...
    human_names=add_specified_vehicles(vehicle_params, edge+"_human", human_veh_left_or_right, human_veh_lane_change)
    rl_names=add_specified_vehicles(vehicle_params, edge+"_rl", rl_veh_left_or_right, rl_veh_lane_change)
...
def add_specified_vehicles(vehicle_params, veh_prefix, veh_right_left_or_both, veh_lane_change):
    # This is for two-lane case, should be able to be generalized
    # veh_right_left_or_both: 
    ...
    for i in range(0, len(veh_names)):
        ...
        if "human" in veh_name and i==0: # the right most lane and human
            ...
            speed_mode=15
            veh_num=0
        operator(vehicle_params, veh_name, speed_mode, veh_num)
    return veh_names
...
def reset_inflows(args, flow_params):
    ...
    if args.human_inflows and args.rl_inflows and args.rl_lane_change and args.human_lane_change and args.merge_inflow:
        ...
        add_veh_and_inflows_to_edge(inflows, veh_params, "inflow_highway", args.rl_inflows, args.rl_lane_change, args.human_inflows, args.human_lane_change)
        add_veh_and_inflows_to_edge(inflows, veh_params, "inflow_merge", [], [], [args.merge_inflow], [0])
